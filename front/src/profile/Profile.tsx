  ...
  const fetchOrders = async (token: string) => {
    try {
      const response = await fetch(
        `http://localhost:8080/user/get-user-orders/${clientData.id}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="p-4">
      <div className="flex sm:gap-40 xs:gap-10 justify-center pb-10">
        <Link to={"/shop"}>
          <h1 className="text-sm font-bold text-primary-500 underline hover:text-secondary-500 pt-3">
            Shop
          </h1>
        </Link>
        <Link to={"/workshop"}>
        ...
        <h1 className="text-2xl font-bold">
          Profile page: {clientData.username}{" "}
        </h1>
        <button
          onClick={logOut}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex justify-center items-center">
          <div className="relative p-5 bg-white rounded shadow-lg">
            <span
              ...
              onClick={() => setIsDataChanged(false)}
            >
              &times;
            </span>
            <div>
              <HText>
                YOUR DATA IS{" "}
                <span className="text-primary-500">SUCCESSFULLY</span> CHANGED
              </HText>
              <div>
      ...
      {orders.map((order) => (
        <div key={order.id} className="mb-6 p-4 border rounded-md shadow-sm">
          ...
          <p className="text-sm text-gray-600 mt-2">{`Order Date: ${order.date.slice(
            0,
            10
          )}`}</p>
