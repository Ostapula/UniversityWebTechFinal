...
const History = ({setSelectedPage}: Props) => {
    ...
    const fetchOrders = async (token: string) => {
        try {
            const response = await fetch(`http://localhost:8080/user/get-user-workshop/${clientData.id}`, {
                method: 'GET',
                headers: {
    ...
    const handleOnClick = async (id: number) => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/user/cancel-workshop/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    return (
        <section id="history"
            className="p-4"
        >
            <motion.div onViewportEnter={() => setSelectedPage(SelectedPageWorkshop.History)}>
                History:
            </motion.div>
            {workshopOrders.map(workshop => (
                <div key={workshop.id}>
                    <HistoryDetail historyItem={workshop} onClick={() => handleOnClick(workshop.id)} btnName={"Cancel"}/>
                </div>
            ))}
        </section>
    )
