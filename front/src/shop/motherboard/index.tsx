...
const Motherboard = ({ setSelectedPage }: Props) => {
    ...
    const handleOnClick = (image: string, price: number, prodId: number) => {
        ...
        dispatch(motherboardChange(image));
        dispatch(motherboardCaseChange(price));
    };

    const fetchProducts = async (): Promise<Product[]> => {
        try {
            const response = await fetch('http://localhost:8080/items/get-motherboard');
            if (!response.ok) {
    ...
    return (
        <section id="motherboard">
            ...
            >
                <HText>
                    MOTHER<span className="text-primary-500">BOARD</span>
                </HText>
            </motion.div>
            <motion.div
            ...
            >
                <motion.div
                    ...
                    onViewportEnter={() => setSelectedPage(SelectedPageShop.Motherboard)}
                >
