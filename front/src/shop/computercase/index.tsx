...
const ComputerCases = ({ setSelectedPage }: Props) => {
  ...
  const fetchProducts = async (): Promise<Product[]> => {
    try {
      const response = await fetch(
        "http://localhost:8080/items/get-computer-cases"
      );
      if (!response.ok) {
  ...
  return (
    <section id="computercase">
      ...
      <motion.div className="flex justify-center pt-10 px-2">
        <motion.div
            ...
            >
              {products.id === 11 ? (
                ...
                <ProductCart
                  ...
                  onClick={() =>
                    handleOnClick(products.image, products.price, products.id)
                  }
                />
              )}
            </motion.div>
