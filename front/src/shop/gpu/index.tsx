...
const GPU = ({ setSelectedPage }: Props) => {
    ...
    const handleOnClick = (image: string, price: number, prodId: number) => {
        ...
        dispatch(gpuChange(image));
        dispatch(gpuCaseChange(price));
    };
    const fetchProducts = async (): Promise<Product[]> => {
        try {
            const response = await fetch('http://localhost:8080/items/get-gpu');
            if (!response.ok) {
    ...
    <section id="gpu">
        ...
        >
            <HText>
                G<span className="text-primary-500">P</span>U
            </HText>
        </motion.div>
        <motion.div
        ...
        >
            <motion.div
                ...
                onViewportEnter={() => setSelectedPage(SelectedPageShop.GPU)}
            >
