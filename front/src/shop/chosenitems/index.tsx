...
const ChosenItems = ({ setSelectedPage }: Props) => {
  const computerCaseChange = useSelector(
    (state: RootState) => state.computercasechanger.value
  );
  const motherboardChanger = useSelector(
    (state: RootState) => state.motherboardchanger.value
  );
  ...
  const memoryChanger = useSelector(
    (state: RootState) => state.memorychanger.value
  );
  const powersupplyChanger = useSelector(
    (state: RootState) => state.powersupplychanger.value
  );
  const coolerChanger = useSelector(
    (state: RootState) => state.coolerchanger.value
  );
  const compSumChange = useSelector(
    (state: RootState) => state.sumchanger.value
  );
  const motherSumChange = useSelector(
    (state: RootState) => state.mothersumchanger.value
  );
  const gpuSumChange = useSelector(
    (state: RootState) => state.gpusumchanger.value
  );
  const cpuSumChange = useSelector(
    (state: RootState) => state.cpusumchanger.value
  );
  const ramSumChange = useSelector(
    (state: RootState) => state.ramsumchanger.value
  );
  const memorySumChange = useSelector(
    (state: RootState) => state.memorysumchanger.value
  );
  const psSumChange = useSelector(
    (state: RootState) => state.powersupplysumchanger.value
  );
  const coolerSumChange = useSelector(
    (state: RootState) => state.coolersumchanger.value
  );
  ...
  const buyConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/user/order/${clientData.id}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <section id="home" className="gap-16 py-20">
      <motion.div
      ...
      >
        ...
        <button
          onClick={clearChosenItems}
          className="rounded-lg bg-secondary-500 xs:px-8 px-3 py-2 hover:text-white hover:bg-primary-500"
        >
          Clear All
        </button>
        <button
          onClick={handleOnClick}
          className="rounded-lg bg-secondary-500 xs:px-10 px-3 py-2 hover:text-white hover:bg-primary-500"
        >
          BUY
        </button>
        {isPopupVisible && (
