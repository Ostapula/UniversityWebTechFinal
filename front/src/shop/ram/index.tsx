...
const RAM = ({ setSelectedPage }: Props) => {
    ...
    const handleOnClick = (image: string, price: number, prodId: number) => {
        ...
        dispatch(ramChange(image));
        dispatch(ramCaseChange(price));
    };

    const fetchProducts = async (): Promise<Product[]> => {
        try {
            const response = await fetch('http://localhost:8080/items/get-ram');
            if (!response.ok) {
    ...
    return (
        <section id="ram">
            ...
            >
                <HText>
                    <span className="text-primary-500">RA</span>M
                </HText>
            </motion.div>
            <motion.div
            ...
            >
                <motion.div
                    ...
                    onViewportEnter={() => setSelectedPage(SelectedPageShop.RAM)}
                >
