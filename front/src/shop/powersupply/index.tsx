...
const PowerSupply = ({ setSelectedPage }: Props) => {
    ...
    const handleOnClick = (image: string, price: number, prodId: number) => {
        ...
        dispatch(powersupplyChange(image));
        dispatch(powersupplyCaseChange(price));
    };

    const fetchProducts = async (): Promise<Product[]> => {
        try {
            const response = await fetch('http://localhost:8080/items/get-power-supply');
            if (!response.ok) {
    ...
    return (
        <section id="powersupply">
            ...
            >
                <HText>
                    <span className="text-primary-500">Power</span> Supply
                </HText>
            </motion.div>
            <motion.div
            ...
            >
                <motion.div
                    ...
                    onViewportEnter={() => setSelectedPage(SelectedPageShop.PowerSupply)}
                >
